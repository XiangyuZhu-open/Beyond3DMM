...
class PRNet_PAF_Vis_Shape(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, kernal_size_paf =3):
        ...
        self.mask_conv = nn.Sequential( #*[
            *padding_same_conv2d(256, 1, in_channels, kernel_size=3, stride=1),  # 256x256x3
            nn.BatchNorm2d(in_channels, eps=0.001, momentum=0.001),
            nn.ReLU(inplace=True)
            # ]
        )

        self.paf_conv = nn.Sequential(  # *[
            *padding_same_conv2d(256 * kernal_size_paf, in_channels, in_channels, kernel_size=kernal_size_paf, stride=kernal_size_paf),  # 256x256x3
            # ]
        )

        self.input_conv = nn.Sequential(  # *[
            *padding_same_conv2d(256, in_channels + 3, size, kernel_size=4, stride=1),  # 256x256x16
