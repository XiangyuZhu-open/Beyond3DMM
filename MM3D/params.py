...
d = make_abs_path('model/')
keypoints = np.load(osp.join(d, 'keypoints_sim.npy'))
w_shp = np.load(osp.join(d, 'w_shp_sim.npy'))
w_exp = np.load(osp.join(d, 'w_exp_sim.npy'))  # simplified version
meta = np.load(osp.join(d, 'param_whitening.pkl'), allow_pickle=True)
